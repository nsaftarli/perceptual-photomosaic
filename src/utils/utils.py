...
def per_class_acc(model_dir, data='validation', imgrows=224, imgcols=224,
                  textrows=28, textcols=28, chars=16):
...
def get_label(label_path, textrows, textcols, dims):
    ...
    Output: 2D array of indices, where each index is a number that represents
            a character specified by char_dict
    '''
    # print(label_path)
    f = open(label_path, 'r')
