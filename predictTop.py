...
def predictTop(argmax, templates, patch_size=8, batch_size=1, rgb=False, num_temps=62, img_size=224, softmax_h=None, softmax_w=None):
    reshaped_argmax = tf.reshape(argmax,[-1, softmax_h * softmax_w, num_temps])
    ##########################################################################################################
    ...
    if not rgb:
        with tf.name_scope('output_and_tile'):
            ...
            output = tf.reshape(tf.transpose(tf.reshape(
                output, [batch_size, softmax_h, softmax_w, patch_size, patch_size]),
